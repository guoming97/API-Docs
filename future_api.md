...
- 访问频率最快为100ms间隔
- 访问不同站点的币对时,需要在请求的header里添加from = '标识'
...
### 申请API Key
用户的API权限在网站的个人中心->我的API内获取。点击申请API即可获得，其中API Key是交易所提供给API用户的访问密钥，API Secret是用于对请求参数签名的私钥。
...
key = 'your key'
secret = 'your secret'
url = "https://api.cloudservicex.com/order/active"
symbol = 'BTC_USDT'
nonce = int(time.time())
...
let url = "https://api.cloudservicex.com/order/active"
let symbol = 'BTC_USDT'
...
- 示例
```
curl -H 'key: xxx' -H 'sign: yyy' -H 'version: 2.0' -X POST https://api.cloudservicex.com/contract/symbol/list -d 'nonce=1536826456'
```
...
- 示例
```
curl -H 'key: xxx' -H 'sign: yyy' -H 'version: 2.0' -X POST https://api.cloudservicex.com/future/account/balance/list -d 'nonce=1536826456'
```
...
- 示例
```
curl -H 'key: xxx' -H 'sign: yyy' -H 'version: 2.0' -X POST https://api.cloudservicex.com/future/account/transfer_margin -d 'nonce=1536826456&currency=BTCUSD&amount=-0.3302'
```
...
- 示例
```
curl -H 'key: xxx' -H 'sign: yyy' -H 'version: 2.0' -X POST https://api.cloudservicex.com/contract/order -d 'nonce=1536826456&symbol=FUTURE_BTCUSD&side=1&type=1&price=3500&amount=1&leverage=10'
```
...
- 示例
```
curl -H 'key: xxx' -H 'sign: yyy' -H 'version: 2.0' -X POST https://api.cloudservicex.com/contract/remove -d 'nonce=1536826456&symbol=FUTURE_BTCUSD&order_id='
```
...
- 示例
```
curl -H 'key: xxx' -H 'sign: yyy' -H 'version: 2.0' -X POST https://api.cloudservicex.com/contract/close -d 'nonce=1536826456&symbol=FUTURE_BTCUSD&price=3600'
```
...
- 示例
```
curl -H 'key: xxx' -H 'sign: yyy' -H 'version: 2.0' -X POST https://api.cloudservicex.com/contract/activeorders -d 'nonce=1536826456&page=1&size=10'
```
...
- 示例
```
curl -H 'key: xxx' -H 'sign: yyy' -H 'version: 2.0' -X POST https://api.cloudservicex.com/contract/orderhistory -d 'nonce=1536826456&page=1&size=10'
```
...
- 示例
```
curl -H 'key: xxx' -H 'sign: yyy' -H 'version: 2.0' -X POST https://api.cloudservicex.com/future/account/orderfills -d 'nonce=1536826456&page=1&size=10'
```